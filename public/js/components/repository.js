myApp.component('dataRepository',{
    ...
    controller: function($http,$scope,$rootScope) {
        ...
        $scope.dataset.then(function(response){
            ...
            for(let item in response.data){
                    ...
                    for(let x in response.data[item]['Study_design']){
                        ...
                        if(response.data[item]['Study_design'][x].Name=='Animal experiment'){
                            response.data[item]['Organism']=response.data[item]['Study_design'][x]['Organism'];
                            organism_types.push(response.data[item]['Study_design'][x]['Organism'])
                        }
                }
            };
            ...
            $scope.organism_unique=organism_types.filter((x,i,a)=>a.indexOf(x)===i)
            });
        ...
        $scope.addFilterOrganism=function(x){
            if($scope.filterorganism.includes(itemvalue)==false){
              $scope.filterorganism.push(itemvalue);
            }
        ...
        $scope.filterOrganism=function(x){
            if($scope.filterorganism.length!=0){
                return $scope.filterorganism.includes(x.Organism)
            }else{
                return true
            }
        }
